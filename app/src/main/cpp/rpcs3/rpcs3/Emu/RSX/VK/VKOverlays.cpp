...
namespace vk
{
	...
	void overlay_pass::init_descriptors()
	{
		...
		CHECK_RESULT(_vkCreatePipelineLayout(*m_device, &layout_info, nullptr, &m_pipeline_layout));
	}
	...
	void overlay_pass::load_program(vk::command_buffer& cmd, VkRenderPass pass, const std::vector<vk::image_view*>& src)
	{
		...
		_vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, program->pipeline);
		...
		_vkCmdBindVertexBuffers(cmd, 0, 1, &buffers, &offsets);
	}
	...
	void overlay_pass::destroy()
	{
		if (initialized)
		{
			...
			_vkDestroyDescriptorSetLayout(*m_device, m_descriptor_layout, nullptr);
	...
	{
		_vkCmdDraw(cmd, num_drawable_elements, 1, first_vertex, 0);
	}
	...
	void overlay_pass::set_up_viewport(vk::command_buffer& cmd, u32 x, u32 y, u32 w, u32 h)
	{
		...
		vp.minDepth = 0.f;
		vp.maxDepth = 1.f;
		_vkCmdSetViewport(cmd, 0, 1, &vp);

		VkRect2D vs = { { static_cast<s32>(x), static_cast<s32>(y) }, { w, h } };
		_vkCmdSetScissor(cmd, 0, 1, &vs);
	}
		...
		vk::data_heap& upload_heap, u64 key, u32 w, u32 h, u32 layers, bool font, bool temp, const void* pixel_src, u32 owner_uid)
	{
		...
		_vkCmdCopyBufferToImage(cmd, upload_heap.heap->value, tex->value, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
	...
	void ui_overlay_renderer::update_uniforms(vk::command_buffer& cmd, vk::glsl::program* /*program*/)
	{
		...
		_vkCmdPushConstants(cmd, m_pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT, 0, 68, push_buf);
		...
		_vkCmdPushConstants(cmd, m_pipeline_layout, VK_SHADER_STAGE_FRAGMENT_BIT, 68, 12, push_buf);
	}
	...
	void attachment_clear_pass::update_uniforms(vk::command_buffer& cmd, vk::glsl::program* /*program*/)
	{
		...
		_vkCmdPushConstants(cmd, m_pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT, 0, 32, data);
	}
	...
	void attachment_clear_pass::set_up_viewport(vk::command_buffer& cmd, u32 x, u32 y, u32 w, u32 h)
	{
		...
		_vkCmdSetScissor(cmd, 0, 1, &region);
	}
	...
	{
		_vkCmdPushConstants(cmd, m_pipeline_layout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, 16, config.data);
	}
