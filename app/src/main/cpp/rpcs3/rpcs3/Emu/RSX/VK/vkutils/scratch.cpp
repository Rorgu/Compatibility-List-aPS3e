...
namespace vk
{
	...
	VkSampler null_sampler()
	{
		...
		_vkCreateSampler(*g_render_device, &sampler_info, nullptr, &g_null_sampler);
		return g_null_sampler;
	}
	...
	vk::image_view* null_image_view(const vk::command_buffer& cmd, VkImageViewType type)
	{
		...
		_vkCmdClearColorImage(cmd, tex->value, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &clear_color, 1, &range);
	...
	vk::buffer* get_scratch_buffer(const vk::command_buffer& cmd, u64 min_required_size, bool zero_memory)
	{
		...
		if (init_mem || zero_memory)
		{
			...
			_vkCmdFillBuffer(cmd, buf->value, 0, zero_length, 0);
	...
	void clear_scratch_resources()
	{
		...
		if (g_null_sampler)
		{
			_vkDestroySampler(*g_render_device, g_null_sampler, nullptr);
