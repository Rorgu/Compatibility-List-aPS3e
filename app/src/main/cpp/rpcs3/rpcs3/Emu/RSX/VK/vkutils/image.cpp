...
namespace vk
{
	...
	image::~image()
	{
		_vkDestroyImage(m_device, value, nullptr);
	}
	...
	void image::create_impl(const vk::render_device& dev, u32 access_flags, const memory_type_info& memory_type, vmm_allocation_pool allocation_pool)
	{
		...
		CHECK_RESULT(_vkCreateImage(m_device, &info, nullptr, &value));

		VkMemoryRequirements memory_req;
		_vkGetImageMemoryRequirements(m_device, value, &memory_req);
		...
		{
			CHECK_RESULT(_vkBindImageMemory(m_device, value, device_mem, memory->get_vk_device_memory_offset()));
	...
	{
		_vkDestroyImageView(m_device, value, nullptr);
	}
