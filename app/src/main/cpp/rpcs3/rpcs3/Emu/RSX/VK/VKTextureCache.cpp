...
namespace vk
{
		...
		if (require_gpu_transform)
		{
			...
			if (require_tiling)
			{
				...
				if (valid_range.start != dma_sync_region.start || real_pitch != tiled_region.tile->pitch)
				{
					...
					_vkCmdCopyBuffer(cmd, dma_mapping.second->value, working_buffer->value, 1, &mem_load);
				...
				if (g_cfg.video.renderdoc_compatiblity)
				{
					...
					// Debug write
					auto scratch_img = vk::get_typeless_helper(VK_FORMAT_R8G8B8A8_UNORM, RSX_FORMAT_CLASS_COLOR, tiled_region.tile->pitch / 4, 768);
			...
			if (rsx_pitch == real_pitch) [[likely]]
			{
				...
				_vkCmdCopyBuffer(cmd, working_buffer->value, dma_mapping.second->value, 1, &copy);
			}
			else
			{
				dma_sync(true);
				...
				_vkCmdCopyBuffer(cmd, working_buffer->value, dma_mapping.second->value, transfer_height, copy.data());
			}
		}
		else
		{
			...
			_vkCmdCopyImageToBuffer(cmd, src->value, src->current_layout, dma_mapping.second->value, 1, &region);
		}
	...
	void texture_cache::copy_transfer_regions_impl(vk::command_buffer& cmd, vk::image* dst, const std::vector<copy_region_descriptor>& sections_to_transfer) const
	{
		...
		for (const auto& section : sections_to_transfer)
		{
			...
			if (src_w == section.dst_w && src_h == section.dst_h) [[likely]]
			{
				...
				_vkCmdCopyImage(cmd, src_image->value, src_image->current_layout, dst->value, dst->current_layout, 1, &copy_rgn);
			}
			else
			{
				...
				if (_dst != dst) [[unlikely]]
				{
					...
					_vkCmdCopyImage(cmd, _dst->value, _dst->current_layout, dst->value, dst->current_layout, 1, &copy_rgn);
				}
			}
		...
		const std::vector<copy_region_descriptor>& sections_to_copy, const rsx::texture_channel_remap_t& remap_vector)
	{
		...
		if (!(dst_aspect & VK_IMAGE_ASPECT_DEPTH_BIT))
		{
			...
			_vkCmdClearColorImage(cmd, image->value, image->current_layout, &clear, 1, &dst_range);
		}
		else
		{
			...
			else
			{
				...
				if (flags & texture_create_flags::initialize_image_contents)
				{
					...
					if (image->aspect() & VK_IMAGE_ASPECT_COLOR_BIT)
					{
						...
						_vkCmdClearColorImage(cmd, image->value, image->current_layout, &color, 1, &range);
					}
					else
					{
						VkClearDepthStencilValue clear{ 1.f, 255 };
						_vkCmdClearDepthStencilImage(cmd, image->value, image->current_layout, &clear, 1, &range);
					}
				}
			}
		}
	...
	void texture_cache::cleanup_after_dma_transfers(vk::command_buffer& cmd)
	{
		...
		if (cmd.access_hint != vk::command_buffer::access_type_hint::all)
		{
			...
			CHECK_RESULT(_vkResetCommandBuffer(cmd, 0));
			cmd.begin();
		}
		else
		{
    ...
    vk::viewable_image* texture_cache::upload_image_simple(vk::command_buffer& cmd, VkFormat format, u32 address, u32 width, u32 height, u32 pitch)
	{
		...
		_vkGetImageSubresourceLayout(*m_device, image->value, &subresource, &layout);
