...
namespace vk
{
	...
	void compute_task::init_descriptors()
	{
		...
		CHECK_RESULT(_vkCreatePipelineLayout(*g_render_device, &layout_info, nullptr, &m_pipeline_layout));
	}
	...
	void compute_task::destroy()
	{
		if (initialized)
		{
			...
			_vkDestroyDescriptorSetLayout(*g_render_device, m_descriptor_layout, nullptr);
			_vkDestroyPipelineLayout(*g_render_device, m_pipeline_layout, nullptr);
	...
	void compute_task::load_program(const vk::command_buffer& cmd)
	{
		...
		_vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, m_program->pipeline);
	...
	void compute_task::run(const vk::command_buffer& cmd, u32 invocations_x, u32 invocations_y, u32 invocations_z)
	{
		...
		load_program(cmd);
		_vkCmdDispatch(cmd, invocations_x, invocations_y, invocations_z);
	}
	...
	void cs_shuffle_base::set_parameters(const vk::command_buffer& cmd, const u32* params, u8 count)
	{
		...
		_vkCmdPushConstants(cmd, m_pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0, count * 4, params);
	}
